...
const MEMORY_LOG_STORE = globalThis.__NO_GRIP_LOG_STORE__ || (globalThis.__NO_GRIP_LOG_STORE__ = []);
  ...
  if (store && typeof store.get === 'function') {
    ...
    if (raw) {
      try {
        ...
        // Fall back to memory if the KV payload is unreadable.
      }
    }
    return [];
  }
  ...
  async fetch(request, env, ctx) {
    ...
    if (method === 'GET' && url.pathname === '/status') {
      const rows = (await Promise.all(
        Object.keys(VALID_STUDENT_IDS)
          .sort()
          .map((studentId) => statusRow(studentId, env))
      )).join('');
