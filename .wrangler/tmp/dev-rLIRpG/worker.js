...
// .wrangler/tmp/bundle-HARefM/checked-fetch.js
...
// .wrangler/tmp/bundle-HARefM/strip-cf-connecting-ip-header.js
...
var SPEEDTEST_HTML = `<!DOCTYPE html>
<html>
<head>
  <meta charset="UTF-8">
...
function speedtestResult(target) {
  ...
  const output = `PING ${cleanTarget} (${cleanTarget}) 56(84) bytes of data.
...
__name(speedtestResult, "speedtestResult");
function renderSpeedtestPage(target) {
  const cleanTarget = sanitize(target || "8.8.8.8");
  const { command, output } = speedtestResult(cleanTarget);
  return SPEEDTEST_HTML.replace("{{TARGET}}", escapeForHtml(cleanTarget)).replace("{{RESULT}}", escapeForHtml(`${command}
${output}`));
}
__name(renderSpeedtestPage, "renderSpeedtestPage");
...
// .wrangler/tmp/bundle-HARefM/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-HARefM/middleware-loader.entry.ts
